<?php
  header('Content-Type:text/html;charset=UTF-8');
?>
<div class="header_bar">
  <div class="container">
    <div class="collect lf">
      <a href="javascript:;" ><i class="icon"></i>收藏慕课</a>
    </div>
    <div class="login rt">
      <span>
        <a href="login.html" id="welcome">你好，请登录 </a>
        <a href="register.html" id="free_register">| 免费注册</a>
      </span>
    </div>
</div>
</div>
<div class="header_search">
  <div class="container">
    <div class="logo lf">
      <a href="index.html"></a>
    </div>
    <div class="search lf">
      <input type="text" class="search_input lf" id='searchInp'/>
      <ul id='search_result'>
      </ul>
      <input type="button" class="search_btn lf" value="搜 索"/>
    </div>
    <div class="cart rt">
      <a id='settle_up' class="cart_text lf">
        <i class="icon"></i>
        <span>去购物车结算</span>
      </a>
    </div>
  </div>
</div>
<div class="header_nav">
  <div class="container">
    <div class="nav_products lf">
      <h3>
        <a href="javascript:;">
          全部商品分类&nbsp;
        </a>
      </h3>
      <!--商品分类列表-->
      <div class="item_class">
        <div class="item_class_nav">
          <dl>
            <dt>
              <a href="javascript:;">手机</a>
              <a href="javascript:;">数码</a>
              <a href="javascript:;" class="tip">合约机</a>
              <i class="icon"></i>
            </dt>
            <dd>
              <a href="javascript:;">荣耀3X</a>
              <a href="javascript:;">单反</a>
              <a href="javascript:;">智能设备</a>
            </dd>
          </dl>
          <dl>
            <dt>
              <a href="javascript:;">手机</a>
              <a href="javascript:;">数码</a>
              <a href="javascript:;" class="tip">合约机</a>
              <i class="icon"></i>
            </dt>
            <dd>
              <a href="javascript:;">荣耀3X</a>
              <a href="javascript:;">单反</a>
              <a href="javascript:;">智能设备</a>
            </dd>
          </dl>
          <dl>
            <dt>
              <a href="javascript:;">手机</a>
              <a href="javascript:;">数码</a>
              <a href="javascript:;" class="tip">合约机</a>
              <i class="icon"></i>
            </dt>
            <dd>
              <a href="javascript:;">荣耀3X</a>
              <a href="javascript:;">单反</a>
              <a href="javascript:;">智能设备</a>
            </dd>
          </dl>
          <dl>
            <dt>
              <a href="javascript:;">手机</a>
              <a href="javascript:;">数码</a>
              <a href="javascript:;" class="tip">合约机</a>
              <i class="icon"></i>
            </dt>
            <dd>
              <a href="javascript:;">荣耀3X</a>
              <a href="javascript:;">单反</a>
              <a href="javascript:;">智能设备</a>
            </dd>
          </dl>
          <dl>
            <dt>
              <a href="javascript:;">手机</a>
              <a href="javascript:;">数码</a>
              <a href="javascript:;" class="tip">合约机</a>
              <i class="icon"></i>
            </dt>
            <dd>
              <a href="javascript:;">荣耀3X</a>
              <a href="javascript:;">单反</a>
              <a href="javascript:;">智能设备</a>
            </dd>
          </dl>
        </div>
        <div class="item_class_products">
          <div class="item_class_show">
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <div class="item_class_btn">
              <a>电脑整机频道</a>
              <a>硬件/外设频道</a>
            </div>
          </div>
          <div class="item_class_show">
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">装机配件</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">整机附件</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑外设</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">网络设备</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">音频设备</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <div class="item_class_btn">
              <a>电脑整机频道</a>
              <a>硬件/外设频道</a>
            </div>
          </div>
          <div class="item_class_show">
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <div class="item_class_btn">
              <a>电脑整机频道</a>
              <a>硬件/外设频道</a>
            </div>
          </div>
          <div class="item_class_show">
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <div class="item_class_btn">
              <a>电脑整机频道</a>
              <a>硬件/外设频道</a>
            </div>
          </div>
          <div class="item_class_show">
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑外设</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">网络设备</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">音频设备</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <dl>
              <dt>
                <a href="javascript:;">电脑整机</a>
              </dt>
              <dd>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
                <a href="javascript:;">笔记本</a>
                <a href="javascript:;">超极本</a>
                <a href="javascript:;">上网本</a>
                <a href="javascript:;">平板电脑</a>
                <a href="javascript:;">台式机</a>
              </dd>
            </dl>
            <div class="item_class_btn">
              <a>电脑整机频道</a>
              <a>硬件/外设频道</a>
            </div>
          </div>
        </div>
      </div>
    </div>
    <ul class="nav_list lf">
      <li class="on"><a href="filterpage.html">数码城</a></li>
      <li><a href="productcategory.html">天黑黑</a></li>
      <li><a href="fooditems.html">美味食品</a></li>
      <li><a href="javascript:;">发现</a></li>
      <li><a href="javascript:;">二手特卖</a></li>
      <li><a href="javascript:;">名品会</a></li>
  </ul>
  </div>
</div>